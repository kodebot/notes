...
Sometimes we will have different basis vector than a standard one. So, we need a way to convert a vector to and from one vector space to another. So, we can convert the vector to more familiar vector space, perform all operation we need to perform then change the result to original vector space.
